...
function constructPanoramaBtns(objectsForPanorama) {
    panoramaBtns.forEach(n => map.controls.remove(n));
    panoramaBtns = [];
    ...
    objectsForPanorama.forEach((obj, i) => {
        var panoramaButton = new ymaps.control.Button({
            data: {
                id: obj.id,
                content: "смотреть панораму " + obj.id,
                title: 'кликните чтобы посмотреть панораму'
            }
        });
        panoramaBtns.push(panoramaButton);
        ...
        panoramaButton.events.add('click', (e) => {
            var id = e.originalEvent.target.data.get('id');
            findPanorama(getPoints()[id]);
            setTimeout(() => {
